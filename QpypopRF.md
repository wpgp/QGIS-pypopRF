...
At its core, the plugin leverages the rasterio library [@Gillies et al., 2019] for seamless raster input and output operations, ensuring efficient handling of geospatial covariates. For data computation in a vector format, pandas [@pandas development team, 2020] and its spatial extension, geopandas [@Jordahl et al., 2020], enable powerful data manipulation and analysis, particularly with vector data that might be associated with the raster information. To optimise performance and manage the workflow efficiently, joblib [@The joblib developers, 2025] is integrated for pipelining and parallel processing, which significantly speeds up computation by utilizing multi-core processors. Finally, the foundational machine learning capabilities within pypopRF are powered by the extensive algorithms and tools provided by the scikit-learn (sklearn) library [@Pedregossa et al., 2011], allowing for the construction and application of sophisticated models.
